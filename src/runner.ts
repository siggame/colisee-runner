...
import { async_foreach, coroutine, delay, send } from "./helpers";
...
export class Runner {
    ...
    public async run(): Promise<void> {
        ...
        const result_logger = coroutine(async function*(): AsyncIterableIterator<void> {
            const game = yield;
            const { id, status } = await game;
            console.log(id, status);
        });
        // push games to the logger
        send(played_games, result_logger());
    }
